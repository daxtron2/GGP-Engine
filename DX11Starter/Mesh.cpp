...
void Mesh::Init(Vertex* vertices, int vertexCount, unsigned int* indices, int indexCount, ComPtr<ID3D11Device> device)
{
	...
	ibd.ByteWidth = sizeof(unsigned int) * indexBufferLength;	// 3 = number of indices in the buffer
...
Mesh::Mesh(Vertex* vertices, int vertexCount, unsigned int* indices, int indexCount, ComPtr<ID3D11Device> device)
{
	Init(vertices, vertexCount, indices, indexCount, device);
}

Mesh::Mesh(const char* objFile, ComPtr<ID3D11Device> device)
{
